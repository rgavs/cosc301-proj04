...
clone(void(*fcn)(void*), void *arg, void *stack)
{
    ...
    if(proc->thread == 1)         // if parent is also a thread, its parent is a primary process
    ...
    *np->tf = *np->parent->tf;
    ...
    for(i = 0; i < NOFILE; i++)
      if(np->parent->ofile[i])
        np->ofile[i] = filedup(np->parent->ofile[i]);
    np->cwd = idup(np->parent->cwd);
